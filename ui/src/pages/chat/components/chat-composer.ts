...
function renderChatPrimaryActions(props: ChatRunControlsProps) {
  ...
  return html`
    ${
      props.voiceActive && props.onToggleVoice ? html` <openclaw-tooltip .content=${t("chat.composer.stopVoiceInput")}> <button class="chat-send-btn chat-send-btn--stop" @click=${props.onToggleVoice} aria-label=${t("chat.composer.stopVoiceInput")}>${icons.stop} <span class="agent-chat__control-label">${t("chat.composer.stopVoiceInput")}</span></button> </openclaw-tooltip>
            ${abortAction} ` : props.canAbort ? html` ${hasComposedContent ? html` <openclaw-tooltip .content=${t("chat.runControls.queue")}> <button class="chat-send-btn" @click=${storeDraftAndSend} ?disabled=${!props.connected || props.sending} aria-label=${t("chat.runControls.queueMessage")}>${icons.send} <span class="agent-chat__control-label">${t("chat.runControls.queue")}</span></button> </openclaw-tooltip> ` : nothing}
              <openclaw-tooltip .content=${t("chat.runControls.stop")}> <button class="chat-send-btn chat-send-btn--stop" @click=${props.onAbort} aria-label=${t("chat.runControls.stopGenerating")}>${icons.stop} <span class="agent-chat__control-label">${t("chat.runControls.stop")}</span></button> </openclaw-tooltip> ` : true /* 产品需求：默认总是展示发送按钮（语音能力已移除，隐藏语音按钮后此处恒为 true，直接展示发送） */ ? html` <openclaw-tooltip .content=${props.isBusy ? t("chat.runControls.queue") : t("chat.runControls.send")}> <button class="chat-send-btn" @click=${storeDraftAndSend} ?disabled=${!props.connected || props.sending} aria-label=${props.isBusy ? t("chat.runControls.queueMessage") : t("chat.runControls.sendMessage")}>${icons.send} <span class="agent-chat__control-label">${props.isBusy ? t("chat.runControls.queue") : t("chat.runControls.send")}</span></button> </openclaw-tooltip> ` : nothing /* 产品需求：隐藏语音输入按钮（无语音能力）。原代码以注释形式保留，便于回退：
