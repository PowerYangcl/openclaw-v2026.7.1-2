 ...
 * 切回要复原。放内存则「关掉窗格再打开」就丢（拆分视图里很常见），
 * 放服务端则要给网关加一个前端交互态的表 —— 都不合适，localStorage 分桶最贴合。
 *
 * ⚠️ 但**整页刷新（F5）不等价于「换窗格」**，口径是「刷新即作废」：
 *   - **不执行** —— 刷新后那一轮往往早已在网关侧结束（服务端不因刷新中断 run），
 *     把队列补发出去等于替用户做决定；
 *   - **不展示** —— 捞回来的任务在当前页没有任何执行入口，挂在界面上只会让人困惑；
 *   - **不提示** —— 「已恢复 N 条待执行任务，正在继续执行」在这种情况下就是假话
 *     （任务根本不会执行）。
 * 落地方式：`discardPendingTasksFromPreviousPage()` 在**整页加载后的第一次**读盘前
 * 把落盘内容整体作废（由 ChatPane 的 `watch(pendingTaskBucket)` 在 immediate 求值时调用）。
 * 所以本模块的持久化现在只服务「同一个页面周期内跨窗格 / 跨会话复原」。
 *
 ...
 * 症状就是「刷新后待执行任务列表不再展示」（注意：刷新后**本来就不该**恢复队列，
 * 见上面的「刷新即作废」；但同页内重开窗格 / 换会话时仍要能把队列 hydrate 回来，
 * 所以 key 细化的处理不能省）。
 *
 ...
 * 本页生命周期是否已经历过「首次队列读盘」。
 *
 * ⚠️ 必须是**模块级**：整页刷新（F5 / 重新打开页面）会重新执行本模块 ⇒ 重置为 false；
 * 而拆分视图新建窗格、侧栏换会话都不会。这正是「刷新」与「同页内复用」的判别依据
 * —— 组件局部变量区分不了这两者（新建窗格的局部变量同样是全新的）。
 */
let pageLifeStarted = false;

/**
 * 整页加载后的**第一次**调用：把上一个页面周期落盘的队列整体作废，返回 `true`；
 * 之后（同页内的窗格重建 / 换会话）返回 `false`，照常读盘。
 *
 * ## 为什么刷新要作废（口径 2026-09-21 二轮）
 * 用户手动刷新后：队列**不执行**（那一轮早已在网关侧结束，补发等于替用户做决定）、
 * **不展示**（捞回来的任务在当前页没有执行入口）、**不提示**（提示语在这种情况下是假的）。
 * 直接清掉落盘内容，是这三条最省事的落地方式 —— 读盘拿不到东西，自然既不展示也不续跑。
 *
 * ## 为什么清**整份**存储而不是「当前算出来的那个桶 key」
 * 桶 key 会从 `agent:main:<rest>` 细化成 `agent:<实际 agentId>:<rest>`（见文件头那条
 * 时间线），且细化发生在首次读盘之后。只清当前桶的话，旧任务会在 key 细化那一拍
 * 从**另一个桶**被读回来 —— 「刷新后不该展示」就漏了。
 *
 * 代价：同一个人开第二个标签页会顺手清掉第一个标签页落盘的队列
 * （第一个标签页内存里仍有队列，且任何变更都会重新落盘）。这是「刷新即作废」的
 * 必然代价，可接受。
 */
export function discardPendingTasksFromPreviousPage(): boolean {
  if (pageLifeStarted) return false;
  pageLifeStarted = true;
  const storage = getLocalStorage();
  if (storage) {
    try {
      storage.removeItem(PENDING_TASKS_STORAGE_KEY);
    } catch {
      // 隐私模式 / 配额异常：清不掉就退化成「这一次刷新仍会读到旧队列」，不阻塞交互
    }
  }
  return true;
}
